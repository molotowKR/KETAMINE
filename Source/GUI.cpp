...
void GUI::renderChamsWindow(Config& config, bool contentOnly) noexcept
{
    ...
    if (ImGui::Combo("", &currentCategory, "0Enemies\0Planting\0Defusing\0Local player\0Weapons\0Hands\0Backtrack\0Sleeves\0"))
        material = 1;
    ...
    constexpr std::array categories{ "Enemies", "Planting", "Defusing", "Local player", "Weapons", "Hands", "Backtrack", "Sleeves" };
...
void GUI::renderStyleWindow(Config& config, bool contentOnly) noexcept
{
    ...
    if (ImGui::Combo("Menu colors", &config.style.menuColors, "Dark\0Light\0Classic\0"))
        updateColors(config);
    ImGui::PopItemWidth();
        ...
        if (ImGui::BeginPopup("Config to reset")) {
            ...
            for (int i = 0; i < IM_ARRAYSIZE(names); i++) {
                ...
                if (ImGui::Selectable(names[i])) {
                    ...
                    case 9: config.getFeatures().inventoryChanger.reset(memory); config.getFeatures().inventoryChanger.scheduleHudUpdate(); break;
...
{
    ImGui::Begin("ketamine", nullptr, windowFlags | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysAutoResize);
    ...
    if (ImGui::BeginTabBar("TabBar", ImGuiTabBarFlags_Reorderable | ImGuiTabBarFlags_FittingPolicyScroll | ImGuiTabBarFlags_NoTooltip)) {
        ...
        if (ImGui::BeginTabItem("C13H16ClNO")) {
